...
import classes from './CashierOrdersList.module.css';
...
export function CashierOrdersList( { onRefreshRef }: CashierOrdersListProps) {
  ...
  const refreshOrders = () => {
    api.Orders.getAllOrders().then(res => {
      const formattedOrders: RowData[] = res.data.map((order: IOrder) => ({
        ...
        screeningTime: new Date(order.screening.screeningTime).toLocaleString(),
        email: order.email
      }));
      setOrders(res.data);
       ...
       <Modal opened={opened} onClose={close} title="More Details" size="lg">
        {selectedOrder && (
          <div>
            ...
            <Text size="lg" fw={500} mt="xl" mb="md">Screening Details</Text>
